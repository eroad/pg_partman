...
IF v_type IN ('time-static', 'time-dynamic', 'time-custom') THEN
    ...
    WHERE h.inhparent = '||quote_literal(p_parent_table)||'::regclass
      AND c.relname ~ trim(n.nspname::text ||''.'' from (h.inhparent::regclass || ''_p''|| replace(regexp_replace(regexp_replace(regexp_replace('''||v_datetime_string||''',''YYYY|IYYY'',''\d{4}'',''g''),''(MI|IW|MM|DD|HH24|SS)'',''\d{2}'',''g''),''Q'',''\d{1}''),''"'','''') || ''$''))
